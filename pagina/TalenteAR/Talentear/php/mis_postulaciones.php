...
// -------------------------------------------------------------------------------------
// --- 2. Obtener datos del Talento (ID y Nombre) ---
// -------------------------------------------------------------------------------------
...
if ($id_talento > 0) {
    ...
    $notifCountQ = "
        ...
        WHERE c.usuario_id = ?
        AND n.leida = 0
        AND n.estado IN ('invitacion', 'aceptada', 'rechazada')
    ";
    ...
    // 3.2. CONSULTA PRINCIPAL: Trae TODAS las notificaciones (Invitaciones y Postulaciones)
    $notifQ = "
        SELECT
            ...
            cst.titulo AS titulo_casting /* Agregamos el título del casting */
        FROM notificaciones n
        JOIN curriculums c ON n.curriculum_id = c.curriculum_id
        JOIN agencias e ON n.agencia_id = e.id_agencia
        LEFT JOIN castings cst ON n.casting_id = cst.casting_id /* LEFT JOIN para que traiga invitaciones sin casting */
        WHERE c.usuario_id = ?
...
<div class="navbar-links">
    ...
    <div class="notif-container">
        ...
        <div id="notifDropdown" class="notif-dropdown">
            ...
            <hr>
            <?php
// Consulta para el Dropdown (usaremos el array $solicitudes que ya está completo)
if ($id_talento > 0 && !empty($solicitudes)) {
    foreach ($solicitudes as $notif) {
        $leida = $notif['leida'] ? 'opacity:0.6;' : '';
        $estado_notif = $notif['estado'] ?? 'pendiente';
        $casting_id_notif = (int) ($notif['casting_id'] ?? 0);
        
        // Filtro: Solo mostramos si NO FUE LEÍDA O si es una invitación pendiente (para que el talento pueda actuar)
        if ($notif['leida'] == 1 && $estado_notif !== 'invitacion' && $casting_id_notif === 0) {
            continue; // Oculta invitaciones respondidas y leídas.
        }
        if ($notif['leida'] == 1 && $estado_notif === 'pendiente' && $casting_id_notif > 0) {
            continue; // Oculta postulaciones pendientes que ya fueron leídas.
        }
        ...
        } elseif ($estado_notif === 'pendiente' && $casting_id_notif > 0 && $notif['leida'] == 0) {
            echo "<p style='margin:0;'>Tu postulación a un Casting está en revisión por **" . htmlspecialchars($notif['nombre_agencia']) . "**.</p>";
            echo "<p style='color:#4863a0; margin-top:3px;'>Pendiente de revisión...</p>";
        } else {
             // Si no cumple ninguna condición relevante para mostrar, salta
             continue;
        }
...
<div class="container-body">
    ...
    <div class="seccion-solicitudes">
        ...
        <?php endif; ?>
    </div> <div class="seccion-solicitudes">
